...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 2; i++) {
      fg[0] += INFLU_SOLVER *CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      AD<double> psides0 = CppAD::atan(polydereval(coeffs, x0));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, vector<double>& mpc_x, vector<double>& mpc_y) {
  ...
  // NOTE: Feel free to change this to something else.
  // Decceleration is set to 0.1 so that the car doesn't
  // stop frequently
